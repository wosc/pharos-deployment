...
CREATE TABLE `mailboxes` (
  ...
  `uid` int(11) NOT NULL default '<%=node["etc"]["passwd"]["Debian-exim"]["uid"]%>',
  `gid` int(11) NOT NULL default '<%=node["etc"]["passwd"]["Debian-exim"]["gid"]%>',
  PRIMARY KEY  (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8;
...
INSERT INTO users (login, password, role) VALUES ('root', ENCRYPT('<%=node["mailserver"]["thyrida_root_pass"]%>', 'as'), 'root');
